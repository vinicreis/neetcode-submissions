class Solution {
    ...
    private fun dp(s: String, t: String): Int {
        val memo = hashMapOf<Pair<Int, String>, Int>()

        fun dfs(i: Int = 0, cur: String = ""): Int {
            if (cur == t) return 1
            if (cur.length > t.length) return 0
            if (i > s.lastIndex) return 0
            if ((i to cur) in memo) return memo[i to cur]!!

            val result = dfs(i + 1, cur) + dfs(i + 1, cur + s[i])

            memo[i to cur] = result
