    ...
    private fun intuition(s1: String, s2: String, s3: String): Boolean {
        fun dfs(m: Int = 0): Boolean {
            ...
            for (t in s3.indices) {
                ...
                //     t >= m + s2.length -> println("Comparing last indices on s1: s3[$t] != s1[${t - s2.length}]")
                //     else -> println("Comparing middle indices on s2: s3[$t] != s2[${t - m}]")
                // }

                when {
                    t < m -> if (s3[t] != s1[t]) return false
                    t >= m + s2.length -> if (s3[t] != s1[t - s2.length]) return false
