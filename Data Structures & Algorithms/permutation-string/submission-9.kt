class Solution {
    ...
    /**
        ...
        it does not ensure the order, which is not important
    ...
    private fun solution2(s1: String, s2: String): Boolean {
        ...
        for (c in s2) {
            s2Frequencies[c] = s2Frequencies.getOrDefault(c, 0) + 1
        }

        return s1Frequencies.all { (k, v) -> s1Frequencies.containsKey(k) && s2Frequencies[k] == v }
    }
