class Solution {
    ...
    private fun dp(nums: IntArray, target: Int): Int {
        ...
        fun dfs(i: Int = 0, remaining: Int = target): Int {
            ...
            if (memo[i][remaining + total] > Int.MIN_VALUE) return memo[i][remaining + total]

            memo[i][remaining + total] = dfs(i + 1, remaining + nums[i]) + dfs(i + 1, remaining - nums[i])

            return memo[i][remaining + total]
        }
